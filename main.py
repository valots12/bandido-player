    ...
    if len(cnts_sort) != 0:
        ...
        for i in range(len(cnts_sort)):
            if (cnt_is_card[i] == 1):
                ...
                # generates a flattened 200x300 image of the card.
                ...
                if len(history.keys())==0:
                    ...
                    # after 50 consecutive choices of the same move
                    if history[name]==50:
                        num = int(name[5::])
                        if num<3:
...
# test = cv2.threshold(cards[k].warp,50,255,cv2.THRESH_BINARY)[1]
